...
class DataHub(object):
    ...
    def _init_data(self, **params):
        ...
        if self.task == 'regression': 
            if len(np.array(self.data['raw_target']).shape) > 1 and np.array(self.data['raw_target']).shape[1] > 1:
                target = np.array(self.data['raw_target'])[:,-1].reshape(-1,1).astype(np.float32)
