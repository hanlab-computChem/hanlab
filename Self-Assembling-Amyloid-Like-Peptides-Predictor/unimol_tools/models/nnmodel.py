...
from .loss import MAEwithNan, MSEwithNan, weighted_MSEwithNan, weighted_MAEwithNan, weighted_MSEwithNan_Quantile
...
LOSS_RREGISTER = {
    ...
    'multilabel_regression': {
        "mae": MAEwithNan,
        "mse": MSEwithNan,
        "weighted_mae": weighted_MAEwithNan,
        "weighted_mse": weighted_MSEwithNan,
        "weighted_mse_quantile": weighted_MSEwithNan_Quantile
    },
}
...
class NNModel(object):
    ...
    def __init__(self, data, trainer, **params):
        ...
        if self.task == 'multilabel_classification':
            ...
            if self.loss_key is None:
                self.loss_key = 'mae'
            if self.loss_key in ["weighted_mae", "weighted_mse", "weighted_mse_quantile"]:
                self.loss_func = LOSS_RREGISTER[self.task][self.loss_key](torch.Tensor(params["label_weight"]).to(trainer.device))
            else:
                self.loss_func = LOSS_RREGISTER[self.task][self.loss_key]
        else:
    ...
    def run(self):
        ...
        for fold, (tr_idx, te_idx) in enumerate(self.splitter.split(X, y, group)):
            ...
            _y_pred = self.trainer.fit_predict(
                self.model, traindataset, validdataset, 
                self.loss_func, self.activation_fn, self.save_path, 
                fold, self.target_scaler)
