...
def MSEwithNan(y_pred, y_true):
    """
    Calculates the Mean Squared Error (MSE) loss, ignoring NaN values in the target.

    :param y_pred: (torch.Tensor) Predicted values.
    :param y_true: (torch.Tensor) Ground truth values, may contain NaNs.
    
    :return: (torch.Tensor) MSE loss computed only on non-NaN elements.
    """
    mask = ~torch.isnan(y_true)
    y_pred = y_pred[mask]
    y_true = y_true[mask]
...
class weighted_MAEwithNan:
    def __init__(self, weights):
        """
        Initializes the weighted_MAEwithNan class with the specified weights.

        :param weights: (torch.Tensor) Weights for each label.
        """
        self.weights = weights
    
    def __call__(self, y_pred, y_true):
        """
        Calculates the weighted Mean Absolute Error (MAE) loss, ignoring NaN values in the target.

        :param y_pred: (torch.Tensor) Predicted values.
        :param y_true: (torch.Tensor) Ground truth values, may contain NaNs.
        
        :return: (torch.Tensor) Weighted MAE loss computed only on non-NaN elements.
        """
        # mask = ~torch.isnan(y_true)
        # y_pred = y_pred[mask]
        # y_true = y_true[mask]
        y_pred = torch.where(torch.isnan(y_true), torch.tensor(0.0, device=y_true.device), y_pred)
        y_true = torch.where(torch.isnan(y_true), torch.tensor(0.0, device=y_true.device), y_true)
...
class weighted_MSEwithNan:
    def __init__(self, weights):
        """
        Initializes the weighted_MSEwithNan class with the specified weights.
    ...
    def __call__(self, y_pred, y_true):
        """
        Calculates the weighted Mean Squared Error (MSE) loss, ignoring NaN values in the target.

        :param y_pred: (torch.Tensor) Predicted values.
        :param y_true: (torch.Tensor) Ground truth values, may contain NaNs.
        
        :return: (torch.Tensor) Weighted MSE loss computed only on non-NaN elements.
        """
        y_pred = torch.where(torch.isnan(y_true), torch.tensor(0.0, device=y_true.device), y_pred)
        y_true = torch.where(torch.isnan(y_true), torch.tensor(0.0, device=y_true.device), y_true)
...
class weighted_MSEwithNan_Quantile:
    def __init__(self, weights, quantiles=[0.85, 0.95]):
        """
        Initializes the weighted_MSEwithNan_Quantile class with the specified weights and quantile.

        :param weights: (torch.Tensor) Weights for each label.
        :param quantile: (float) Quantile value (tau) between 0 and 1.
        """
        self.weights = weights
    ...
    def __call__(self, y_pred, y_true):
        """
        Calculates the weighted Quantile loss, ignoring NaN values in the target.

        :param y_pred: (torch.Tensor) Predicted values.
        :param y_true: (torch.Tensor) Ground truth values, may contain NaNs.

        :return: (torch.Tensor) Weighted Quantile loss computed only on non-NaN elements.
        """
        y_pred = torch.where(torch.isnan(y_true), torch.tensor(0.0, device=y_true.device), y_pred)
        y_true = torch.where(torch.isnan(y_true), torch.tensor(0.0, device=y_true.device), y_true)

        errors = y_true - y_pred
        quantile_loss = torch.stack([torch.where(errors >= 0, quantile * errors, (quantile - 1) * errors) for quantile in self.quantiles], dim=-1)
        weighted_quantile_loss = quantile_loss * self.weights.unsqueeze(0)
        return weighted_quantile_loss.mean()
        

# def weighted_MAEwithNan(y_pred, y_true, weights):
#     """
#     Calculates the weighted Mean Absolute Error (MAE) loss.

#     :param y_pred: (torch.Tensor) Predicted values.
#     :param y_true: (torch.Tensor) Ground truth values.
#     :param weights: (torch.Tensor) Weights for each label.
    
#     :return: (torch.Tensor) Weighted MAE loss.
#     """
#     mask = ~torch.isnan(y_true)
#     y_pred = y_pred[mask]
#     y_true = y_true[mask]
...
# def weighted_MSEwithNan(y_pred, y_true, weights):
#     """
...
#     :return: (torch.Tensor) Weighted MSE loss.
#     """
#     mask = ~torch.isnan(y_true)
#     y_pred = y_pred[mask]
#     y_true = y_true[mask]
