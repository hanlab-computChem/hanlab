...
###    print "Analysing %d/%d residues" % (ca.n_atoms, universe.atoms.n_residues)
    coor = ca.coordinates()   
    sele = vars()["selection"]
    if "CA" in sele and "N" in sele and "C" in sele and "N" in sele:
        sele = vars()["selection"]
        resi = sele.split()[-1]
        resiS = int(resi.split(':')[0])
        resiE = int(resi.split(':')[1])
       # print sele, resi, resiS, resiE        
        com_coord = []
        for resin in range(resiS, resiE+1):
            if "segid" in sele:
                chn = sele.split('segid')[1].split()[0]
                sele_resin = 'segid %s and resid %s and (name CA or name N or name C or name O)'%(chn,resin)
            else:
                sele_resin = 'resid %s and (name CA or name N or name C or name O)'%(resin)
            com_coord.append(universe.select_atoms(sele_resin).center_of_mass())
        coor = np.array(com_coord)
        
    twist, bending_angles, height, rnou, origins, local_helix_axes, local_screw_angles = \
        main_loop(coor, ref_axis=ref_axis)
